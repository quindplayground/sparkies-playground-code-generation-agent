...
@patch("template_project.flows.portafolios_dynamodb.load.get_logger")
@patch("template_project.flows.portafolios_dynamodb.load.DynamoDBLoader")
...
@patch("template_project.flows.portafolios_dynamodb.load.get_logger")
@patch("template_project.flows.portafolios_dynamodb.load.DynamoDBLoader")
def test_load_with_default_region(
    mock_loader_class,
    mock_get_logger,
    spark,
    mock_vars_resource,
    ...
    """Test load uses default region when not specified in config."""
    mock_logger = MagicMock()
    mock_get_logger.return_value = mock_logger

    del mock_vars_resource.vars.output.region
...
@patch("template_project.flows.portafolios_dynamodb.load.logger")
@patch("template_project.flows.portafolios_dynamodb.load.DynamoDBLoader")
